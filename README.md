...
```
https://github.com/yuanbenio/universe-go-sdk
```
...
| category       | string  | 分类集, 以逗号分隔 "新闻, 商业"                      |用户传入|
| dna            | string  | metadata dna                             |系统生成|
| parent_dna            | string  | 该metadata修改前的dna                             |用户传入，如果是修改前一个metadata的数据，则需要传入前一个metadata的dna|
...
| extra          | map[string]interface{}  | 扩展内容,自定义内容                               |用户传入|
| license        | object  | 许可证                                      |用户传入|
| license.type   | string  | 许可证类型                                    |用户传入|
| license.parameters | map[string]string  | 许可证参数对象,自定义内容                            |用户传入|
| source         | string  | 原内容的链接, article,image的官网或者内容的链接                |用户传入|
| data           | map[string]interface{}  | 存放和原数据相关的内容           |用户传入|
...
**NOTE** 方法的使用请查看test包下的[样例](https://github.com/yuanbenio/universe-go-sdk/tree/master/test)
